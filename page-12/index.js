...
horseLoader.load('/assets/Horse.glb', function ( gltf ) {

	const horse = gltf.scene;
	const child0 = horse.children[0];

	child0.name = 'horse';
	child0.scale.set(0.016, 0.016, 0.016);
	child0.position.y = -1.5;
	earthGroup.add( child0 );
} );
...
const animate = () => {
	...
	const mouseVector = new THREE.Vector2(mousePosition.x / window.innerWidth * 2 - 1, - (mousePosition.y / window.innerHeight) * 2 + 1);

	raycaster.setFromCamera(mouseVector, camera);
	const intersects = raycaster.intersectObjects(scene.children);

	if (earthMaterial.normalMap) {
		// earthMaterial.normalMap.offset.x = mouseVector.x
		// earthMaterial.normalMap.offset.y = mouseVector.y;
	}
